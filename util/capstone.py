...
def run_many_prms_parallel(input_dirs, token,
                           auth_host='https://auth.virtualwatershed.org/api',
                           model_host='https://model.virtualwatershed.org/api',
                           clobber=True,
                           verify_ssl=False):

    def _run_prms(input_dir):

        cl = ModelApiClient(token, auth_host, model_host)
        api = DefaultApi(api_client=cl)
        ...
        data_path = os.path.join(input_dir, 'data.nc')
        param_path = os.path.join(input_dir, 'parameter.nc')
        control_path = os.path.join(input_dir, 'incline_village.control')
    ...
    p = Pool(cpu_count)
    mr_ids = p.map(_run_prms, input_dirs)
